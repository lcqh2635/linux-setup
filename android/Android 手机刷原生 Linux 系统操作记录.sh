...
# 这是目前最通用的方法。思路是在手机上运行一个 VNC 服务器，然后在电脑上用客户端连接并查看。
sudo apt update && sudo apt install tightvncserver
# 安装完成后，运行 vncserver 命令来初始化并设置一个密码。
记下终端输出的信息，比如 New 'X' desktop is your-hostname:1，这里的 :1 就是显示编号，对应的端口是 5901 (默认端口5900 + 显示编号)。
...
# 1. 解压（会保留原目录结构）
7z x rootfs-debian-gnome-7.0-trixie.7z
...
sudo apt install -y fastfetch gnome-software gnome-software-plugin-flatpak
